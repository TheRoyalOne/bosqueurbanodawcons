...
class Inicio_model extends CI_Model 
{
		...
		{			 							
			...
			else //no existe el guardabosques, valida su domicilio e insertalo
			{
				...
				if(is_array($guardabosque["NOMBRE"]))
				{
					$guardabosque["NOMBRE"]="";
				}		
				if(is_array($guardabosque["APELLIDO_x0020_PATERNO"]))
				{
					$guardabosque["APELLIDO_x0020_PATERNO"]="";
				}		
				if(is_array($guardabosque["APELLIDO_x0020_MATERNO"]))
				{
					$guardabosque["APELLIDO_x0020_MATERNO"]="";
				}		
				if(is_array($guardabosque["TELEFONO"]))
				{
					$guardabosque["TELEFONO"]="";
				}		
				if(is_array($guardabosque["CELULAR"]))
				{
					$guardabosque["CELULAR"]="";
				}		
				if(is_array($guardabosque["CORREO"]))
				{
					$guardabosque["CORREO"]="";
				}		
				$this->db->query($sql,array($ID__DOMICILIO,$guardabosque["NOMBRE"],$guardabosque["APELLIDO_x0020_PATERNO"],$guardabosque["APELLIDO_x0020_MATERNO"]	
						,	$guardabosque["TELEFONO"],$guardabosque["CELULAR"],$guardabosque["CORREO"]));										
...
//			echo "<br/>".$rowPlanta["CODIGO_x0020_QR"];
		...
		{			 							
			$sql="SELECT ID__EVENTO,concat(substr(FEC_FECHAINICIO,1,10),\" - \",VCH_NOMBREEVENTO) as VCH_NOMBREEVENTO FROM catconf_065_eventos  order by VCH_NOMBREEVENTO desc ";				
		...
		public function verificaSiYaExiste($evento)
		{
			$sql="select count(*) total from traoper_021_registroadopcion where ID__EVENTOADOPCION=".intval($evento);				
			$row = $this->db->query($sql)->row()->total;
			if(intval($row)!=0)
			{
				return true;
			}
