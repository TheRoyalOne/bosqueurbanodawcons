...
function validarFormularioAdopcion()
{
	if(
		...
		$('#AselMunicipio').val() < 0 ||
		$('#AselColonia').val() < 0 ||
		$('#AtxtCalle').val().length === 0 ||
		$('#AtxtReferencia').val().length === 0
		)
		return false;
	else
		return true;	
	...
	{		
		...
		$.ajax({
			...
			success:function(respuesta)
			{
				...
				addRowT("tablaAdoptar",'<tr><td align="center">'+$("#selEspecie option:selected").text()+'</td><td align="center">'+$("#selCodigos option:selected").text()+'</td><td align="center">'+$("#selZona option:selected").text()+'</td><td align="center">'+$("#selEdad option:selected").text()+'</td><td align="center">$'+respuesta+'</td><td align="center"><input type="checkbox" class="bigcheck" onclick="checkGratis('+contadorArboles+')" id="checkbox'+contadorArboles+'"/></td></tr>', true);						
				var arbol={
