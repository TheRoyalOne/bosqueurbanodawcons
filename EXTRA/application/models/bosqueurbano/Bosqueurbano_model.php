...
class Bosqueurbano_model extends CI_Model 
{
        ...
        public function get_reporteFotos($cadena)
        {
			$cadena=str_replace(" ",",",$cadena);
			$query = $this->db->query('

		select a.ID__EVENTO,a.VCH_NOMBREEVENTO,a.FEC_FECHAINICIO,  Arboles_dados , partici
		 from (
				SELECT e.ID__EVENTO,e.VCH_NOMBREEVENTO,e.FEC_FECHAINICIO,  count(*) as Arboles_dados				
				FROM ebu2.catconf_065_eventos e 
				left join traoper_021_registroadopcion r	on e.ID__EVENTO = r.ID__EVENTOADOPCION
                where e.ID__EVENTO in ('.$cadena.')	group by e.ID__EVENTO
                )a
                left join
                (
                select GROUP_CONCAT(distinct(us.VCH_NOMBRE)) as partici,ID__EVENTO from catconf_069_rel_evento_personal per 
                left join catconf_007_usuarios us on per.ID__USUARIO=us.ID__USUARIO where ID__EVENTO in ('.$cadena.')
						group by ID__EVENTO
                )b on a.ID__EVENTO=b.ID__EVENTO
                

				')->result_array();									
			$newarray=[];
			$cadenasplit=explode(",",$cadena);
			$directoryBase = getcwd()."/uploads/Evidencias/";
			foreach($query as $objeto)
			{				
				if(is_dir($directoryBase.$objeto["ID__EVENTO"]."/"))
				{
					$objeto["Cantidad_fotos"] = count(scandir($directoryBase.$objeto["ID__EVENTO"]."/"))-2;
					
				}
				else
				{
					$objeto["Cantidad_fotos"] =0;
				}
				array_push($newarray,$objeto);					
			}
			return $newarray;
		}
		...
		{					
			$query = $this->db->query('SELECT ID__MUNICIPIO,ID__ESTADO,upper(VCH_NOMBRE) VCH_NOMBRE FROM catconf_005_municipios where ID__MUNICIPIO>1500000 order by VCH_NOMBRE;')->result_array();					
			return $query;			
		}	

		public function get_especies()
		{					
			$query = $this->db->query('select ID__ESPECIE, VCH_NOMBRECOMUN as especie from catconf_009_especies order by VCH_NOMBRECOMUN')->result_array();					
			return $query;			
		}	
        public function get_colo()
		{					
			$query = $this->db->query('SELECT ID__COLONIA,ID__MUNICIPIO,upper(VCH_NOMBRE) VCH_NOMBRE, VCH_CODIGOPOSTAL FROM catconf_006_colonias where ID__COLONIA>1500000  order by VCH_NOMBRE ;')->result_array();					
			return $query;			
		}	
		...
		{				
			...
			$this->db->delete('catconf_075_asignacion_guardabosques_embajador', array('ID__EMBAJADOR' => $ID__EMBAJADOR));															
		...
		{														
			...
			$this->setClaveembajador($this->db->insert_id());											
		...
		{						
			$sql="SELECT guarda.ID__GUARDABOSQUE,
				...
				if($ciudad!='')
				{				
					$anteriorCiudad=$ciudad-300000;					
					$queryanteriorCiudad="";
					if($anteriorCiudad-300000<0)
					{
						$queryanteriorCiudad=" or muni.ID__MUNICIPIO=".$anteriorCiudad." ";
					}
					
					$sql.=" and (muni.ID__MUNICIPIO =".$ciudad." ".$queryanteriorCiudad." )";
				}							
...
//			die($this->db->last_query());				
		...
		{									
			...
			$sql="SELECT ID__INVENTARIO,NUM_CANTIDAD,(DATEDIFF(now(),FEC_FECHAGERMINACION)/30) as edad,VCH_NOMBRE as zona,catconf_052_catalogoubicacion.ID__UBICACION,VCH_NOMBRECOMUN as especie,
				 contenedor_nombre as contenedor,procedencia_nombre as procedencia,INT_USO
                 ...
                 where NUM_CANTIDAD >0 and catconf_052_catalogoubicacion.INT_ESTATUS = 1
		...
		{												
			$sql="SELECT ID__UBICACION,VCH_NOMBRE FROM catconf_052_catalogoubicacion where INT_ESTATUS = 1 order by VCH_NOMBRE asc";													  									  
			$query = $this->db->query($sql)->result_array();	
			return $query;			
		}	        
...
//			die($this->db->last_query());
		...
		{						
			...
			foreach($query as $row)
			{
				if($row["VCH_TIPO"]==2)
				{
					$indicetemporal=0;
					$empresanombre="";
					$ARRAYEMPRESAS=JSON_DECODE($row["VCH_EMPRESASREFOR"]);
					foreach ($ARRAYEMPRESAS as $emp)
					{
						...
						$empresanombre.=$this->db->query("select VCH_NOMBREEMPRESA from catconf_013_empresa where ID__EMPRESA=?",array($emp))->row()->VCH_NOMBREEMPRESA;
						$indicetemporal++;
					}											
					$query[$cont]["VCH_NOMBREEMPRESA"]=$empresanombre;
				}
				$cont++;
			}
		...
		{			
			...
			$ID__EVENTO=$this->db->insert_id();
			$this->sendProgresoEvento($ID__EVENTO,0);		
			$this->log($ID__EVENTO,$this->session->userdata["logged_in"]["ID__USUARIO"],"Alta de evento");			
			echo "Guardado exitosamente";
		}
												...
												$FEC_FECHAFIN,$ID__EVENTO,$VCH_LATITUD,$VCH_LONGITUD)
		{
			...
			$this->db->query($sql,
				array($VCH_ESTATUS,$VCH_TIPO,$ID__EMPRESA,$VCH_NOMBREEVENTO,
					  ...
					  $ID__EVENTO));	
			echo "Guardado exitosamente";
			$this->log($ID__EVENTO,$this->session->userdata["logged_in"]["ID__USUARIO"],"Modificacion del evento");			
		}
		...
		{
			$sql="SELECT catconf_065_eventos.ID__EVENTO,VCH_EMPRESASREFOR,
					...
					if(empty($id))																									
					{
						$sql.=" and FEC_FECHAINICIO > DATE_ADD(now(), INTERVAL -7 DAY) and VCH_ESTATUS=1 ";
					}
					if(!empty($id))																									
					{
...
//			die($sql);
		...
		public function get_catalogoEventosAdopcionCuentaRecursos($id)
		{
			$sql="SELECT catconf_065_eventos.ID__EVENTO,VCH_EMPRESASREFOR,
					...
					where 1=1					
						and FEC_FECHAINICIO > DATE_ADD(now(), INTERVAL -7 DAY) and VCH_ESTATUS=1 
					";// VCH_TIPO=1 ";
					if(!empty($id))																									
					{
						$sql.=" and catconf_065_eventos.ID__EVENTO=".$id;
...
//			die($sql);
		...
		public function buscar_zona_con_especie($ID__ESPECIE)
		{
			$sql="select catconf_052_catalogoubicacion.ID__UBICACION,VCH_NOMBRE
			 ...
			 and catconf_052_catalogoubicacion.INT_USO in (1,7,8)
			 group by ID__UBICACION
			 ";	//INT_USO in (1,7,8) Evento Adopción,Reforestación,Evento Adopción Especial
...
//			die($this->db->last_query());
		...
		{			
			...
			$existencias=$this->busquedaInventarioParaDescontarDeAsignacion($ID__UBICACION,$ID__ESPECIE,$contenedor_id,$edad);	
//			die($this->db->last_query());					
			if($cantidadAsignar>$existencias->NUM_CANTIDAD)	
			{							
				$cantidadAsignar=$existencias->NUM_CANTIDAD;								
			}			
			...
			$this->bosqueurbano_model->reduccion_inventarioGlobal($existencias->ID__INVENTARIO,$cantidadAsignar);			
		}
		public function reiniciarAsignacion($ID__EVENTO)
		{						
			$sql="SELECT * FROM catconf_066_relarboladoasignadoaevento where ID__EVENTO=?;";			 
			$rows = $this->db->query($sql,array($ID__EVENTO))->result_array();	
			foreach($rows as $row)
			{
				extract($row);
				$existencias=$this->busquedaInventarioParaDescontarDeAsignacion($ID__UBICACION,$ID__ESPECIE,$contenedor_id,$NUM_EDADMESES);
				$this->alta_inventarioGlobalCombinar($existencias->ID__INVENTARIO,$NUM_CANTIDAD);				
			}			
			$this->db->delete('catconf_066_relarboladoasignadoaevento', array('ID__EVENTO' => $ID__EVENTO));															
		}
		...
		public function busquedaInventarioParaDescontarDeAsignacion($ID__UBICACION,$ID__ESPECIE,$contenedor_id,$edad)
		{
			$sql="SELECT ID__INVENTARIO,NUM_CANTIDAD FROM catconf_064_inventariosglobal where 
					...
					and NUM_CANTIDAD!=0
					";			 
			$query=$this->db->query($sql,array($ID__UBICACION,$ID__ESPECIE,$contenedor_id,$edad))->row();	
			return $query;
		}
...
//			die($this->db->last_query());
		...
		public function get_selectPersonalEvento($id)
		{
			...
			(
				select ID__USUARIO from  catconf_069_rel_evento_personal where ID__EVENTO=?
			) and ID__USUARIO!=1 order by VCH_NOMBRE asc";
			$query = $this->db->query($sql,array($id))->result_array();	
			return $query;
...
//			die($this->db->last_query());
		...
		{
			$sql="select VCH_QR,FECHA_ALTA from  catconf_071_etiquetas where ID__ESPECIE=? and  ID__EMPRESA=? and VCH_ANIO=? and NUM_PERDIDA=0 and ID__EVENTO is null order by VCH_QR ASC";																	  									  
...
//			die($this->db->last_query());
		...
		public function	AltaEtiquetas($ID__ESPECIE,$ID__EMPRESA,$VCH_ANIO,$NUM_CANTIDAD)
		{
			...
			if($ID__ESPECIE<10)
			{
				//$ID__ESPECIE=sprintf('%02d', $ID__ESPECIE);
				$ID__ESPECIE="0".$ID__ESPECIE;
			}

			$contador=0;						
			$sql="select VCH_QR,SUBSTRING_INDEX( VCH_QR, '-', -1 ) a from  catconf_071_etiquetas where ID__ESPECIE=? and VCH_ANIO=? order by SUBSTRING_INDEX( VCH_QR, '-', -1 ) desc limit 1";																	  									  			
			$data=$this->db->query($sql,array($ID__ESPECIE,$VCH_ANIO))->row();
			if(!empty($data))
			{
				
				$qr=$VCH_ANIO."-".$ID__ESPECIE."-".sprintf('%06d', $data->a);
				$inicio=$qr;
				//$inicio= $data->VCH_QR;	
			...
			{
				echo "No se generaron mas etiquetas ya que el stock reportado es superior o igual al numero de este evento";
			}
...
/*			$sql="select rel.ID__ESPECIE,VCH_NOMBRECOMUN as especie,ubicacion.VCH_NOMBRE as ubicacion,arboladosolicitado as cantidadarboles,count(*) as cantidadetiquetas from catconf_071_etiquetas et
	...
	*/					
			...
			$ID__EMPRESA = $this->db->query("select ID__EMPRESA from catconf_065_eventos where ID__EVENTO=?",array($id))->row()->ID__EMPRESA;	
			...
			$sql="select ID__EVENTO,especies.ID__ESPECIE,VCH_NOMBRECOMUN as especie,sum(NUM_CANTIDAD) as cantidadarboles from 
				...
				group by especies.ID__ESPECIE order by VCH_NOMBRECOMUN asc
							";
			$query = $this->db->query($sql,array($id))->result_array();				
			$i=0;
			foreach($query as $row)
			{
				$query[$i]["cantidadetiquetas"] = $this->db->query("select count(*) as total from catconf_071_etiquetas where ID__ESPECIE=? and  ID__EMPRESA=?",array($row["ID__ESPECIE"],$ID__EMPRESA))->row()->total;				
				$i++;
			}			
			return $query;
		...
		public function BusquedaInventarioEtiquetasDisponiblesConFiltro($ID__ESPECIE,$ID__EVENTO)
		{
			...
			if($VCH_TIPO==1)
			{			
				$sql="select count(*) as cuantidad from catconf_071_etiquetas where ID__ESPECIE=? and  ID__EMPRESA=?  and  NUM_PERDIDA=0 and NUM_USADA=0";			 
				$query = $this->db->query($sql,array($ID__ESPECIE,$ID__EMPRESA))->row()->cuantidad;	
			}
			else
			{					
	...
	//						die($this->db->last_query());
		...
		public function get_registrotalleres($ID__TALLER, $VCH_TALLER)
		{
			...
			$sql="SELECT ID__CVETALLER,VCH_TALLER,NUM_SESIONES,NUM_CONVOCADOS,VCH_INSTALACIONES,VCH_PRECIO,VCH_NOMBREEMPRESA,TALLERISTA,fecha FROM catconf_057_registrotalleres r
left join catconf_013_empresa e on r.ID__PATROCINADOR=e.ID__EMPRESA 
left join 
	( select GROUP_CONCAT(VCH_NOMBRE,',') TALLERISTA,ID__CLAVETALLER from catconf_059_talleristasxtaller GROUP BY ID__CLAVETALLER
    )T ON T.ID__CLAVETALLER=r.ID__CVETALLER
left join
	(
		select GROUP_CONCAT(FEC_FECHA,' ') fecha,ID__CLAVETALLER from catconf_058_convocadostalleres GROUP BY ID__CLAVETALLER
    )f on r.ID__CVETALLER=f.ID__CLAVETALLER
     where ".$idt." VCH_TALLER like '%".$VCH_TALLER."%' order by ID__CVETALLER desc limit 100";
			$query = $this->db->query($sql)->result_array();	
//			die($this->db->last_query());
			return $query;	
		}
		...
		{
			$ID__EMPRESA=$this->db->query("select ID__EMPRESA from catconf_065_eventos where ID__EVENTO=?",array($ID__EVENTO))->row()->ID__EMPRESA;	
			$sql="select 		
			ID__ETIQUETA,VCH_QR,et.ID__ESPECIE,VCH_NOMBRECOMUN as especie,et.ID__EMPRESA,emp.VCH_NOMBREEMPRESA as empresa 
			from catconf_071_etiquetas et inner join catconf_009_especies especies on et.ID__ESPECIE=especies.ID__ESPECIE
			inner join catconf_013_empresa emp on et.ID__EMPRESA=emp.ID__EMPRESA 			
			where et.ID__EMPRESA =? and  NUM_USADA=0 and et.ID__ESPECIE IN (SELECT ID__ESPECIE FROM catconf_066_relarboladoasignadoaevento WHERE ID__EVENTO=? )";																	  									  
			$query = $this->db->query($sql,array($ID__EMPRESA,$ID__EVENTO))->result_array();	
			return $query;
...
//			die("legue");
		...
		public function get_etiquetasAdopcionCiudadana_filtroespecie($ID__EVENTO)
		{
			$sql="select 		ID__ETIQUETA,VCH_QR,et.ID__ESPECIE,VCH_NOMBRECOMUN as especie,et.ID__EMPRESA,emp.VCH_NOMBREEMPRESA as empresa from catconf_071_etiquetas 
			et inner join catconf_009_especies especies on et.ID__ESPECIE=especies.ID__ESPECIE
			inner join catconf_013_empresa emp on et.ID__EMPRESA=emp.ID__EMPRESA  where NUM_PERDIDA=0 and VCH_ANIO=YEAR(now()) and NUM_USADA=0 and et.ID__ESPECIE=".$ID__EVENTO;																	  									  
			$query = $this->db->query($sql)->result_array();	
			return $query;
		...
		public function get_etiquetasAdopcionOnline($ID__EVENTO)
		{
			...
			$sql="select 		ID__ETIQUETA,VCH_QR,et.ID__ESPECIE,VCH_NOMBRECOMUN as especie,et.ID__EMPRESA,emp.VCH_NOMBREEMPRESA as empresa from catconf_071_etiquetas 
			et inner join catconf_009_especies especies on et.ID__ESPECIE=especies.ID__ESPECIE
			inner join catconf_013_empresa emp on et.ID__EMPRESA=emp.ID__EMPRESA  where NUM_USADA=0 and et.ID__EMPRESA=?";																	  									  
			$query = $this->db->query($sql,array($ID__EMPRESA))->result_array();	
//			die($this->db->last_query());
			return $query;
		...
		{			
			/*$sql="select .ID__ESPECIE,VCH_NOMBRECOMUN as especie,rel.ID__UBICACION, ubicacion.VCH_NOMBRE as ubicacion,rel.contenedor_id,contenedor_nombre as contenedor,(DATEDIFF(now(),FEC_FECHAGERMINACION)/30),sum(NUM_CANTIDAD) as NUM_CANTIDAD
			, CASE (DATEDIFF(now(),FEC_FECHAGERMINACION)/30)
			WHEN (DATEDIFF(now(),FEC_FECHAGERMINACION)/30)>=0 and (DATEDIFF(now(),FEC_FECHAGERMINACION)/30)<=3  THEN MES_TRES  
			WHEN (DATEDIFF(now(),FEC_FECHAGERMINACION)/30)>=4 and (DATEDIFF(now(),FEC_FECHAGERMINACION)/30)<=6  THEN MES_SEIS  
			WHEN (DATEDIFF(now(),FEC_FECHAGERMINACION)/30)>=7 and (DATEDIFF(now(),FEC_FECHAGERMINACION)/30)<=9  THEN MES_NUEVE  
		...
		{			
			$nods["ID__EVENTO"]=$id;						
			$row=$this->db->query("SELECT VCH_NOMBREEVENTO,VCH_NOMBREEMPRESA,FEC_FECHAINICIO FROM catconf_065_eventos ev inner join catconf_013_empresa emp on ev.ID__EMPRESA=emp.ID__EMPRESA where ID__EVENTO=?",array($id))->row();			
			$nods["VCH_NOMBREEVENTO"]=$row->VCH_NOMBREEVENTO;
			$nods["VCH_NOMBREEMPRESA"]=$row->VCH_NOMBREEMPRESA;
			$nods["FEC_FECHAINICIO"]=$row->FEC_FECHAINICIO;			
		...
		public function generaContenidoAdopcionCiudadanaEtiquetas($id)
		{								
			$nods["etiquetas"]=$this->get_etiquetasAdopcionCiudadana_filtroespecie($id);			
			return JSON_ENCODE($nods);
		}
		...
		{	
			...
			foreach($adopciones as $adopcion)	
			{								
				$ID__GUARDABOSQUE= $this->insertarAdopcionCiudadana($guardabosque,$adopcion);					
			}						
			return $ID__GUARDABOSQUE;
		}
		...
		{			
			...
			$row=$this->db->query($sql,array($guardabosque["VCH_NOMBRE"],$guardabosque["VCH_APELLIDOPATERNO"],$guardabosque["VCH_APELLIDOMATERNO"]))->row();
...
//			die(print_r($arboles));				
		...
		{	
			...
			if(($ID__COLONIA!=0)&&($ID__COLONIA!=''))
			{
				$this->edita_Domicilio($ID__DOMICILIO,$ID__COLONIA,$VCH_CALLE,$VCH_ENTRECALLE);
			}					
		...
		{				
			...
			switch($Tipo)
			{
				CASE 0:	//eventos de Adopcion
				{
					...
					if(!empty($fechaInicio))
					{
						$sql.=" and FEC_FECHAINICIO >'".$fechaInicio."  00:00:00'";
					}
					if(!empty($fechafin))
					{
						$sql.=" and FEC_FECHAFIN <='".$fechaInicio."  23:59:59'";
					}
					if($empresa!=-1)
					{
				...
				CASE 1:	//eventos de Reforestacion
				{
					...
					if(!empty($fechafin))
					{
						$sql.=" and FEC_FECHAFIN <='".$fechafin." 23:59:59'";
					}
					if($empresa!=-1)
					{
						$sql.=" and ID__EMPRESA =".$empresa;
...
//					die($this->db->last_query());
				...
				{						
						...
						if($empresa!=-1)
						{
							$where.=" and ID__EMPRESA =".$empresa;
						}
						if($ID__ESPECIE!=-1)
						{							
							$where.=" and e.ID__ESPECIE =".$ID__ESPECIE;
						}		
							
							
					$sql="  select a.ID__ARBOL,VCH_NOMBREEVENTO,FEC_FECHAINICIO,FEC_FECHAFIN,VCH_CODIGOQR,VCH_NOMBRECOMUN,a.FEC_FECHA,a.VCH_LATITUD,a.VCH_LONGITUD ,VCH_ESTADO,VCH_SALUD,VCH_CON_ETIQUETA,VCH_CONTENEDOR,VCH_ACCESO_AL_ARBOL,VCH_RUTA_FOTO_COMPLETA,VCH_RUTA_FOTO_SOLOETIQUETA from 
								(
									SELECT ID__ARBOL,ID__EVENTOADOPCION,VCH_FOTO,FEC_FECHA FROM traoper_021_registroadopcion 
									where FEC_FECHA > '".$fechaInicio." 00:00:00' and FEC_FECHA <'".$fechafin." 23:59:59'
								)reg
								inner join catconf_020_arboles a on reg.ID__ARBOL=a.ID__ARBOL
								left join catconf_065_eventos c on reg.ID__EVENTOADOPCION=c.ID__EVENTO
								inner join catconf_009_especies e on a.ID__ESPECIE=e.ID__ESPECIE
								left join 
								(
...
//echo "<pre>";	die(print_r(($inventarioInicial)));
			...
			//24 OCTUBRE DESACTIVADO PUES YA NO PERTENECEN A EVENTO
		...
		public function setClaveembajador($ID__EMBAJADOR)
		{						
			...
			$sql="select * from catconf_019_embajadores where ID__EMBAJADOR =?";													  									  
			$correo=$this->db->query($sql,array($ID__EMBAJADOR))->row()->VCH_CORREO;	
			//echo $randstring."*";
			$sql="update catconf_019_embajadores  set VCH_PASSWORD=? where ID__EMBAJADOR =?";													  									  
			$this->db->query($sql,array(md5($randstring),$ID__EMBAJADOR));	
			//die($randstring);						
			//echo($randstring);	
						
			$this->sendMailEmbajador($correo,$randstring);
			return; 
		}		
...
<br/>
			...
			$this->email->to('idamy@dwsoftware.mx');
...
//			die($this->db->last_query());
		...
		public function sendMailEmbajador($correo,$clave)
		{		
			
			$html="¡Felicidades! has sido registrado como Embajador Urbano, tu cuenta de acceso es: <br/>
							Usuario: ".$correo."<br/>
					Contraseña: ".$clave."<br/>
					Ahora eres parte del Bosque Urbano, confiamos en tu compromiso…<br/>
					podras Acceder mediante la siguiente liga <a href='http://bosqueurbano.hartec.com.mx/ExtraEmbajador/' target='_blank'>http://bosqueurbano.hartec.com.mx/ExtraEmbajador/</a>
					<br/><br/>
					ATTE.<br/><br/>
					<br/>
					Extra A.C
				...
				// $config['smtp_user']    = 'laura.hartec@gmail.com';
				// $config['smtp_pass']    = 'SJhartec';
				$config['smtp_user']    = 'dagurez@gmail.com';
				$config['smtp_pass']    = '25764310Aa';
				...
				$config['validation'] = TRUE; // bool whether to validate email or not   
				$this->load->library('email',$config);
				$this->email->from('sistemas@bosqueurbanoextra.org.mx', 'Extra A.C');				
				$this->email->to($correo);		
				$this->email->subject('Extra A.C - Embajador urbano.');
				$this->email->message($html);
			    $this->email->set_mailtype("html");			
				$r = $this->email->send();
				echo $this->email->print_debugger();
...
//			die($cuantasgenero."!?!?");
		...
		public function get_etiquetasEventoSinFiltrar($id)
		{
			$ID__EMPRESA=$this->db->query("select ID__EMPRESA from catconf_065_eventos where ID__EVENTO=?",array($id))->row()->ID__EMPRESA;	
			...
			$sql=" SELECT VCH_QR as qr ,VCH_NOMBRECOMUN as especie
				from(
						...
						where ID__EMPRESA=?)a
				left join catconf_009_especies esp on a.ID__ESPECIE=esp.ID__ESPECIE";									  									  
			return $this->db->query($sql,array($ID__EMPRESA))->result_array();	
		}
		
		public function getAsignadosReforestacion($ID__EVENTO)
		{
			$sql=" select esp.ID__ESPECIE,VCH_NOMBRECOMUN from catconf_066_relarboladoasignadoaevento rel 
					left join catconf_009_especies esp on esp.ID__ESPECIE=rel.ID__ESPECIE
				   where ID__EVENTO=?";									  									  
		   $records=$this->db->query($sql,array($ID__EVENTO))->result_array();
//		   die($this->db->last_query());
			return $records;				
		}		
		public function setAsignadosReforestacion($ID__EVENTO,$ID__ESPECIE,$NUM_VIVOS,$NUM_SANOS)
		{			
			$sql="select sum(NUM_CANTIDAD) NUM_CANTIDAD from catconf_066_relarboladoasignadoaevento where ID__EVENTO=? and ID__ESPECIE=?";									  									  
			$totales=$this->db->query($sql,array($ID__EVENTO,$ID__ESPECIE))->row()->NUM_CANTIDAD;							
			$sql="INSERT INTO traoper_055_supervivenciareforesta (ID__EVENTO,ID__ESPECIE,NUM_SANOS,NUM_ENFERMOS,NUM_VIVOS,NUM_MUERTOS) VALUES	(?,?,?,	?,?,?);";									  									  
			$this->db->query($sql,array($ID__EVENTO,$ID__ESPECIE,$NUM_SANOS,($totales-$NUM_SANOS),$NUM_VIVOS,($totales-$NUM_VIVOS)	));			
		}
		
		public function log($ID__EVENTO,$ID__USUARIO,$TIPO_EVENTO)
		{
			$this->db->query("INSERT INTO catconf_078_creacioneventos(ID__EVENTO,ID__USUARIO,TIPO_EVENTO)VALUES(?,?,?)",array($ID__EVENTO,$ID__USUARIO,$TIPO_EVENTO));						
		}
		public function get_ResponsablePrevio($ID__EVENTO)
		{
			$obj=$this->db->query("select    ID__RESPONSABLECARGA as ID__USUARIO,FEC_FECHAYHORACARGA as FFEC_FECHAFIN from catconf_066_relarboladoasignadoaevento where ID__EVENTO=?",array($ID__EVENTO))->row();			
			if(empty($obj))
			{
				$arr["ID__USUARIO"]="1";
				$arr["FFEC_FECHAFIN"]="";
			...
			else
			{
				...
				$arr["ID__USUARIO"]=$obj->ID__USUARIO;					
				$arr["FFEC_FECHAFIN"]=$obj->FFEC_FECHAFIN;
			}
			return $arr;
		}
		
		public function get_especiesAsignadasAeventoInformativo($ID__EVENTO)
		{
			$sql=" select VCH_NOMBRECOMUN,NUM_CANTIDAD from 
					catconf_066_relarboladoasignadoaevento rel
					left join catconf_009_especies esp on rel.ID__ESPECIE =esp.ID__ESPECIE
				   where ID__EVENTO=?";									  									  
		   $records=$this->db->query($sql,array($ID__EVENTO))->result_array();
  		   return $records;							
		...
		public function get_domicilio($ID__EVENTO)
		{
			$sql=" select VCH_CALLE,col.VCH_NOMBRE as colonia,VCH_CODIGOPOSTAL,muni.VCH_NOMBRE as municipio from 
					(SELECT ID__DOMICILIO FROM catconf_065_eventos where ID__EVENTO= ?) a
					left join catconf_008_domicilios domi on a.ID__DOMICILIO=domi.ID__DOMICILIO
					left join catconf_006_colonias col on col.ID__COLONIA=domi.ID__COLONIA
					inner join catconf_005_municipios muni on muni.ID__MUNICIPIO=col.ID__MUNICIPIO
					";									  									  
		   $records=$this->db->query($sql,array($ID__EVENTO))->result_array();
		   if(count($records)>0)
		   {
		...
		public function get_EspeciesParaAsignarArbolado()
		{
			$sql=" 	SELECT inv.ID__INVENTARIO,mos.ID__ESPECIE,VCH_NOMBRECOMUN,NUM_CANTIDAD,inv.contenedor_id, cont.contenedor_nombre,inv.ID__UBICACION , VCH_NOMBRE ubicacion
					,TIMESTAMPDIFF(MONTH, FEC_FECHAGERMINACION, now()) edad
					 FROM catconf_079_especies_a_mostrar mos 
						LEFT JOIN catconf_064_inventariosglobal inv
						left join catconf_009_especies esp on esp.ID__ESPECIE=inv.ID__ESPECIE
						left join catconf_052_catalogoubicacion ubi on ubi.ID__UBICACION=inv.ID__UBICACION
						left join catconf_062_contenedores cont on cont.contenedor_id=inv.contenedor_id
					on mos.ID__ESPECIE=inv.ID__ESPECIE 
					where NUM_CANTIDAD >0  and INT_USO=1 and ubi.INT_ESTATUS = 1
					order by VCH_NOMBRECOMUN,edad,VCH_NOMBRE asc";									  									  
			return 	$this->db->query($sql)->result_array();	
			
		}
		public function cargarListaEspeciesAsignadas($ID__EVENTO)
		{
			$sql=" 	select sum(NUM_CANTIDAD) sumatoria ,VCH_NOMBRECOMUN from catconf_066_relarboladoasignadoaevento rel
			left join catconf_009_especies esp on rel.ID__ESPECIE=esp.ID__ESPECIE 
			where ID__EVENTO=?
			group by esp.ID__ESPECIE
			order by VCH_NOMBRECOMUN asc
			";									  									  
			$val= 	$this->db->query($sql,array($ID__EVENTO))->result_array();	

			return $val;
			
		}
		public function getDatosColoniaOrdenSalida($ID__COLONIA)
		{
			$sql=" 	select col.VCH_NOMBRE as colonia,VCH_CODIGOPOSTAL,muni.VCH_NOMBRE as municipio,est.VCH_NOMBRE as estado 
					from catconf_006_colonias col
					inner join catconf_005_municipios muni on col.ID__MUNICIPIO=muni.ID__MUNICIPIO and col.ID__COLONIA=?
					inner join catconf_004_estados est on muni.ID__ESTADO=est.ID__ESTADO
			";									  									  
			$val= 	$this->db->query($sql,array($ID__COLONIA))->row();	
			return $val;			
		}
}
