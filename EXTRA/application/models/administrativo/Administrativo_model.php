...
class Administrativo_model extends CI_Model 
{
						...
						inner join 		 catconf_001_perfiles 
									...
									where 
									 catconf_007_usuarios.VCH_NOMBRE like '%".$VCH_NOMBRE."%' 
									  ...
									  $sql.="order by VCH_NOMBRE, perfil asc";									  									  
			$query = $this->db->query($sql)->result_array();					
			return $query;			
		}	
		public function get_estados()
		{					
			$query = $this->db->query('SELECT ID__ESTADO,VCH_NOMBRE from catconf_004_estados WHERE EXCLUSIVO=0 order by VCH_NOMBRE')->result_array();					
			return $query;			
		}
		...
		{						
			...
			where ID__USUARIO=?';			
			/*die('update catconf_007_usuarios
			set
			ID__PERFIL='.$ID__PERFIL.',VCH_NOMBRE='.$VCH_NOMBRE.',VCH_APELLIDOPATERNO='.$VCH_APELLIDOPATERNO.',VCH_APELLIDOMATERNO='.$VCH_APELLIDOMATERNO.',VCH_CORREO='.$VCH_CORREO.',
			VCH_TELEFONO='.$VCH_TELEFONO.',VCH_CELULAR='.$VCH_CELULAR.',VCH_PUESTO='.$VCH_PUESTO.',VCH_ESTATUS='.$VCH_ESTATUS.',VCH_USUARIO='.$VCH_USUARIO.',
			VCH_OBSERVACIONES='.$VCH_OBSERVACIONES.',ID__DOMICILIO='.$ID__DOMICILIO.' 
			where ID__USUARIO='.$ID__USUARIO);
			die($sql);*/
			$this->db->query($sql,array($ID__PERFIL,$VCH_NOMBRE,$VCH_APELLIDOPATERNO,$VCH_APELLIDOMATERNO,$VCH_CORREO,
										$VCH_TELEFONO,$VCH_CELULAR,$VCH_PUESTO,$VCH_ESTATUS,$VCH_USUARIO,
