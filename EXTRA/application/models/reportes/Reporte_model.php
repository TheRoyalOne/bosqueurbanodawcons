...
class Reporte_model extends CI_Model 
{
		...
		public function getAllInfoEventosB($fechaInicio,$fechafin,$ID__EVENTO,$ID__EMPRESA,$VCH_ESTATUS)
		{
			...
			$sqltotalarboles=	"SELECT count(*) total FROM catconf_020_arboles 
								where ID__EVENTO in (
									SELECT ID__EVENTO FROM catconf_065_eventos ".$where." )";									
			$totalarboles = $this->db->query($sqltotalarboles)->result_array()[0]["total"];	
			//			die($this->db->last_query());
			$sqltotalarbolesPorEspecie=	"SELECT count(*) total,catconf_020_arboles.ID__ESPECIE,VCH_NOMBRECOMUN FROM catconf_020_arboles
								left join catconf_009_especies on catconf_020_arboles.ID__ESPECIE=catconf_009_especies.ID__ESPECIE
								 where ID__EVENTO in (
														SELECT ID__EVENTO FROM catconf_065_eventos ".$where." 
													)
									group by ID__ESPECIE";									
			$totalarbolesPorEspecie = $this->db->query($sqltotalarbolesPorEspecie)->result_array();

			$sql="SELECT count(distinct(ID__EMPRESA)) as total FROM catconf_065_eventos ".$where;	
			$sql.=" order by VCH_NOMBREEVENTO asc";										  									  
			$totalPatrocinadores = $this->db->query($sql)->result_array()[0]["total"];			
			
			$sql="SELECT count(*) as total FROM catconf_065_eventos ".$where." and VCH_NOMBREEVENTO like '%Tianguis%'";	
			$totaleventosTianguis = $this->db->query($sql)->result_array()[0]["total"];			
			$sql="SELECT count(*) total FROM traoper_021_registroadopcion 
								where ID__EVENTOADOPCION in 
								(
									SELECT ID__EVENTO FROM catconf_065_eventos ".$where." and VCH_NOMBREEVENTO like '%Tianguis%'
								)";	
			$totalArboleseventosTianguis = $this->db->query($sql)->result_array()[0]["total"];							
			$sql="				SELECT count(*) total,VCH_NOMBREEVENTO,FEC_FECHAINICIO,ID__EVENTO FROM traoper_021_registroadopcion 
								left join	catconf_065_eventos on traoper_021_registroadopcion.ID__EVENTOADOPCION=catconf_065_eventos.ID__EVENTO
								where ID__EVENTOADOPCION in 
								(
								...
								";	
			$totalTianguisdia = $this->db->query($sql)->result_array();				
			$sql="				select count(*) total,VCH_NOMBRECOMUN,ID__EVENTO from catconf_009_especies  
								left join catconf_020_arboles on catconf_009_especies.ID__ESPECIE=catconf_020_arboles.ID__ESPECIE
								where ID__EVENTO IN 
								(
								
									SELECT ID__EVENTO FROM catconf_065_eventos ".$where." and VCH_NOMBREEVENTO like '%Tianguis%'
								)								
								group by catconf_009_especies.ID__ESPECIE,ID__EVENTO
								";	
			$totalTianguisdiaEspecie = $this->db->query($sql)->result_array();										
			$sql="SELECT count(*) as total FROM catconf_065_eventos ".$where." and VCH_NOMBREEVENTO like '%zv%' or '%vulnerable%'";	
			$totaleventosZV = $this->db->query($sql)->result_array()[0]["total"];			
			$sql="SELECT count(*) total FROM traoper_021_registroadopcion 
								where ID__EVENTOADOPCION in 
								(
									SELECT ID__EVENTO FROM catconf_065_eventos ".$where." and VCH_NOMBREEVENTO like '%zv%' or '%vulnerable%'
								)";	
			$totalArboleseventosZV = $this->db->query($sql)->result_array()[0]["total"];							
			$sql="				SELECT count(*) total,VCH_NOMBREEVENTO,FEC_FECHAINICIO,ID__EVENTO FROM traoper_021_registroadopcion 
								left join	catconf_065_eventos on traoper_021_registroadopcion.ID__EVENTOADOPCION=catconf_065_eventos.ID__EVENTO
								where ID__EVENTOADOPCION in 
								(
								...
								order by FEC_FECHAINICIO asc
								";	
			$totalZVdia = $this->db->query($sql)->result_array();				
			$sql="				select count(*) total,VCH_NOMBRECOMUN,ID__EVENTO from catconf_009_especies  
								left join catconf_020_arboles on catconf_009_especies.ID__ESPECIE=catconf_020_arboles.ID__ESPECIE
								where ID__EVENTO IN 
								(
								...
								group by catconf_009_especies.ID__ESPECIE,ID__EVENTO
								";	
			$totalZVdiaEspecie = $this->db->query($sql)->result_array();	
			...
			$sql="SELECT count(distinct(catconf_006_colonias.ID__COLONIA)) total
					 FROM catconf_065_eventos 
					left join catconf_008_domicilios on catconf_065_eventos.ID__DOMICILIO=catconf_008_domicilios.ID__DOMICILIO
					left join catconf_006_colonias on catconf_008_domicilios.ID__COLONIA=catconf_006_colonias.ID__COLONIA
					left join catconf_005_municipios on catconf_006_colonias.ID__MUNICIPIO=catconf_005_municipios.ID__MUNICIPIO
				 ".$where." ";	
			$totaleventosColonia = $this->db->query($sql)->result_array()[0]["total"];	
			
				
			//			die($this->db->last_query());	
			$sql="		
					
					select VCH_NOMBREEVENTO,FEC_FECHAINICIO,catconf_065_eventos.ID__EVENTO,count(*) total ,ifnull(catconf_006_colonias.VCH_NOMBRE,'No definido') as colonia, ifnull(catconf_005_municipios.VCH_NOMBRE,'No definido') as municipio from catconf_020_arboles 
					left join catconf_065_eventos  on catconf_020_arboles.ID__EVENTO=catconf_065_eventos.ID__EVENTO
					left join catconf_008_domicilios on catconf_065_eventos.ID__DOMICILIO=catconf_008_domicilios.ID__DOMICILIO
					left join catconf_006_colonias on catconf_008_domicilios.ID__COLONIA=catconf_006_colonias.ID__COLONIA
					left join catconf_005_municipios on catconf_006_colonias.ID__MUNICIPIO=catconf_005_municipios.ID__MUNICIPIO						
					".$where."
					group by catconf_006_colonias.ID__COLONIA,catconf_065_eventos.ID__EVENTO
					order by colonia desc , municipio desc		
								";	
			$totalColoniadia = $this->db->query($sql)->result_array();		
			//die(print_r($totalColoniadia));
			...
			$sql="SELECT VCH_NOMBREEVENTO,ifnull(VCH_LATITUD,0) VCH_LATITUD,ifnull(VCH_LONGITUD,0) VCH_LONGITUD,FEC_FECHAINICIO,FEC_FECHAFIN FROM catconf_065_eventos ".$where;	
			$sql.="";										  									  
			$eventosUbicados = $this->db->query($sql)->result_array();
			...
			$data["totaleventos"]=$totaleventos;
			$data["totalPatrocinadores"]=$totalPatrocinadores;
			$data["totaleventosTianguis"]=$totaleventosTianguis;
			$data["totalArboleseventosTianguis"]=$totalArboleseventosTianguis;
			$data["totalTianguisdia"]=$totalTianguisdia;
			$data["totalTianguisdiaEspecie"]=$totalTianguisdiaEspecie;
			
			$data["totaleventosZV"]=$totaleventosZV;
			$data["totalArboleseventosZV"]=$totalArboleseventosZV;
			$data["totalZVdia"]=$totalZVdia;
			$data["totalZVdiaEspecie"]=$totalZVdiaEspecie;
			
			$data["totaleventosColonia"]=$totaleventosColonia;
			//			$data["totalArboleseventosColonia"]=$totalArboleseventosColonia;
			$data["totalColoniadia"]=$totalColoniadia;
						
			
			//$data["totalEventosPorDia"]=$totalEventosPorDia;
			$data["totalEventosPorPatrocinador"]=$totalEventosPorPatrocinador;				
			$data["eventosUbicados"]=$eventosUbicados;		
			$data["totalarboles"]=$totalarboles;
			

			$data["totalarbolesPorEspecie"]=$totalarbolesPorEspecie;			
		...
		public function getAllInfoAdopciudadana($fechaInicio,$fechafin,$ID__EVENTO,$ID__EMPRESA,$ID__ESPECIE,$listado)
		{
			...
			$data["list"]=$list;	
			//							echo "<pre>";							die(print_r($localizaciones));
			return $data;
		}
		...
		{			
			...
			//$where="where VCH_TIPO=1 and FEC_FECHAINICIO >= '".$fechaInicio." 23:59:59' and '".$fechafin."  23:59:59' >= FEC_FECHAFIN";		
			$where="where VCH_TIPO=1";
			$subquery="where date_create >= '".$fechaInicio." 07:00:00' and '".$fechafin."  23:59:59' >= date_create";
			// $where="where VCH_TIPO=1 and segui.FEC_FECHA_SEGUIMIENTO >= '".$fechaInicio." 23:59:59' and '".$fechafin."  23:59:59' >= segui.FEC_FECHA_SEGUIMIENTO";		
			...
			$sql="select * from 
				(SELECT * FROM traoper_051_seguimientosresumida ".$subquery." group by ID__ARBOL order by  ID__SEGUIMIENTO desc) segui
			...
			$sqlSupervivenciaEspecie="
			select ifnull(arb.ID__ESPECIE,0) ID__ESPECIE,VCH_ESTADO,ifnull(VCH_NOMBRECOMUN,'*NO DISPONIBLE*') VCH_NOMBRECOMUN, count(*) cant
 			from
				(SELECT * FROM traoper_051_seguimientosresumida  ".$subquery." group by ID__ARBOL order by  ID__SEGUIMIENTO desc) segui
				left join catconf_020_arboles arb on arb.ID__ARBOL= segui.ID__ARBOL
				left join traoper_021_registroadopcion adop on adop.ID__ARBOL= segui.ID__ARBOL
			...
			,VCH_RUTA_FOTO_COMPLETA,VCH_RUTA_FOTO_SOLOETIQUETA
				from
					...
					left join catconf_012_guardabosques gua  on gua.ID__GUARDABOSQUE=adop.ID__GUARDABOSQUE
					left join catconf_009_especies esp  on esp.ID__ESPECIE=arb.ID__ESPECIE                                                        				
				".$where." AND ((VCH_RUTA_FOTO_COMPLETA != '' AND VCH_RUTA_FOTO_COMPLETA IS NOT NULL)  OR 
                    (VCH_RUTA_FOTO_SOLOETIQUETA!='' AND  VCH_RUTA_FOTO_SOLOETIQUETA IS NOT NULL))";
		...
		public function get_selectTipoTaller()
		{			
			$sql="SELECT * FROM catconf_055_catalogotalleres ";
			$query = $this->db->query($sql)->result_array();	
			return $query;			
		}	
		public function getAllInfoTaller($fechaInicio,$fechafin,$ID__EMPRESA,$ID__TALLER)
		{			
			$where="where STR_TO_DATE(FEC_FECHA, '%d/%m/%Y') >= '".$fechaInicio." 23:59:59' and '".$fechafin."  23:59:59' >= STR_TO_DATE(FEC_FECHA, '%d/%m/%Y')";											
			if($ID__TALLER!=-1)
			{
				$where.=" and ID__TALLER=".intval($ID__TALLER);
				$nombretipotaller = $this->db->query("select VCH_NOMBRE from catconf_055_catalogotalleres where ID__TALLER=?",array($ID__TALLER))->result_array()[0]["VCH_NOMBRE"];	

			}
			else
			{
				$nombretipotaller = "Todos";	
			}
			if($ID__EMPRESA!=-1)			
			{
				$where.=" and ID__PATROCINADOR=".intval($ID__EMPRESA);
				$nombreempresa = $this->db->query("select VCH_NOMBREEMPRESA from catconf_013_empresa where ID__EMPRESA=?",array($ID__EMPRESA))->result_array()[0]["VCH_NOMBREEMPRESA"];	
			}
			else
			{
			...
			$sql=" SELECT COUNT(DISTINCT(ID__CVETALLER)) TOTAL FROM catconf_057_registrotalleres taller
					left join catconf_058_convocadostalleres horarios
					on taller.ID__CVETALLER=horarios.ID__CLAVETALLER ".$where;										  									  
			$totalTALLERES = $this->db->query($sql)->result_array()[0]["TOTAL"];	
			$sql=" Select count(distinct(ID__PATROCINADOR)) TOTAL from catconf_057_registrotalleres taller
					left join catconf_058_convocadostalleres horarios
					on taller.ID__CVETALLER=horarios.ID__CLAVETALLER ".$where;										  									  
			$totalPatrocinadores = $this->db->query($sql)->result_array()[0]["TOTAL"];	
			
			$sql=" select VCH_TALLER,FEC_FECHA , VCH_HORA,VCH_PRECIO, INT_ASISTENCIA,VCH_NOMBREEMPRESA,col.VCH_NOMBRE as colonia,
mun.VCH_NOMBRE as municipio,VCH_INSTALACIONES
 from catconf_057_registrotalleres taller
					left join catconf_058_convocadostalleres horarios on taller.ID__CVETALLER=horarios.ID__CLAVETALLER
                    left join catconf_013_empresa pat on pat.ID__EMPRESA=taller.ID__PATROCINADOR
                    left join catconf_008_domicilios dom on dom.ID__DOMICILIO=taller.ID__DOMICILIO
			...
			$data=array();											
			$data["nombretipotaller"]=$nombretipotaller;
			$data["nombreempresa"]=$nombreempresa;
			$data["totalTALLERES"]=$totalTALLERES;			
			$data["totalPatrocinadores"]=$totalPatrocinadores;
