						...
						<tr>
							<td align="center">
								<table>
									...
									<tr>
										<td>Mostrar listado <b><?=$dataeventos["nombreempresa"]?></b></td>
			 ...
			 <tr>
				<td align="center" width="100%">
					  <table width="70%" border=0>
						...
						<tr>
							<td width="50px">&nbsp;</td>
							<td class="bordersub"> Patrocinadores</td>
							<td width="20px">
						...
						<tr>
							...
							<td colspan="2">
								<table width="100%">
									<tr>
										<td width="50px">&nbsp;</td>
										<td class="bordersub">Arboles adoptados</td>
										<td width="20px">
											<b><b><?=$dataeventos["totaladoptados"]?></b></b>
										</td>	 
									</tr>
								</table>
							</td>
					  ...
					  <table width="70%" border=0>
						<tr>
							<td align="left" colspan="2" >
								<b>Fecha de Adopcion</b>
							</td>													
						</tr>
						<?php
						foreach($dataeventos["totalporfecha"] as $dia)
						{
						?>
						<tr>
							<td width="50px">&nbsp;</td>
			...
			<tr>
				<td align="center">
					  <table width="70%" border=0>
						<tr>
							<td align="left" colspan="2" >
								<b>Adopciones por evento</b>
							</td>						
							
						</tr>
						<?php
//					echo "<pre>";		die(print_r($dataeventos["totalporevento"]));
						foreach($dataeventos["totalporevento"] as $dia)
						{
						?>
						<tr>
							<td width="50px">&nbsp;</td>
							<td class="bordersub"><?=$dia["VCH_NOMBREEVENTO"];?></td>
							<td width="20px">
								<b><?=$dia["NUM_CANTIDAD"]?></b>
							</td>	 
						</tr>						
			...
			<tr>
				<td align="center">
					  <table width="70%" border=0>
						<tr>
							<td align="left" colspan="2" >
								<b>Adopciones por patrocinador</b>
							</td>						
							
						</tr>
						<?php
						//die(print_r($dataeventos["totalporpatronos"]));
						foreach($dataeventos["totalporpatronos"] as $dia)
						{
						?>
						<tr>
							<td width="50px">&nbsp;</td>
			...
			<tr>
				<td align="center">
					  <table width="70%" border=0>
						<tr>
							<td align="left" colspan="2" >
								<b>Adopciones por especie</b>
							</td>						
							
						</tr>
						<?php
						foreach($dataeventos["totalporespecie"] as $dia)
						{
						?>
						<tr>
							<td width="50px">&nbsp;</td>
			...
			<tr>
				<td align="center">
					<!--<img width="900" src="https://maps.googleapis.com/maps/api/staticmap?center=Albany,+NY&zoom=13&scale=1&size=600x300&maptype=roadmap&format=png&visual_refresh=true" alt="Google Map of Albany, NY">-->
			...
			<tr>
				<td align="center">
					  <table width="70%" border=1>
						...
						<?php 						
						foreach($dataeventos["list"] as $lista)
						{
						?>
						<tr>							
							<td align="center">
		...
		function initMap() 
		{
			...
			<?php
			foreach($dataeventos["localizaciones"] as $dia)
			{				
				if(!empty($dia["VCH_LATITUD"]))
				{
					?>
					if(GEOCERCA!='')
					{
							...
							{
								addMarkerb({lat: parseFloat(<?=utf8_decode($dia["VCH_LATITUD"])?>), lng: parseFloat(<?=utf8_decode($dia["VCH_LONGITUD"])?>)}); 
							}
					}
					else
					{

						addMarkerb({lat: parseFloat(<?=$dia["VCH_LATITUD"]?>), lng: parseFloat(<?=$dia["VCH_LONGITUD"]?>)}); 
					}								
			<?php
				}
			}
		...
		var chartContainer2=Morris.Donut({
			  ...
			  <?php 						
						foreach($dataeventos["totalporevento"] as $pat)
						{
						?>
						
						{label: "<?=$pat["VCH_NOMBREEVENTO"]?>", value: <?=$pat["NUM_CANTIDAD"]?>},
						<?php
						}?>			  				
			  ], colors: [
				'#E87B2F','#4270C1','#A2A2A2',
				'#FABC00','#5e7df9','#b2cfb1',
				'#7e48be','#ab2f10','#4e8f95',
				'#c10bb8','#53219f','#48c83a',
				'#50a6f2','#e0e77a','#1214ca',
				'#a10cde','#ce87ac','#a8715a',
				'#852d1d','#77600e','#ec9b7a',
				'#559b36','#79af6d','#e688aa',
				'#9c541a','#e0dcf9','#5aa97f',
				'#7ff114','#958b06','#364a76',
				'#ea9670','#486e6a','#6cae58'
		...
		var chartContainer3=Morris.Donut({
			  ...
			  <?php 						
						foreach($dataeventos["totalporpatronos"] as $pat)
						{
						?>
						
						{label: "<?=$pat["VCH_NOMBREEMPRESA"]?>", value: <?=$pat["cant"]?>},
						<?php
						}?>			  				
			  ], colors: [
								'#E87B2F','#4270C1','#A2A2A2',
		...
		var chartContainer4=Morris.Donut({
			  ...
			  <?php 						
						foreach($dataeventos["totalporespecie"] as $pat)
						{
						?>
						
						{label: "<?=$pat["VCH_NOMBRECOMUN"]?>", value: <?=$pat["cant"]?>},
						<?php
						}?>			  				
			  ], colors: [
								'#E87B2F','#4270C1','#A2A2A2',
		...
		Morris.Bar({
		 ...
		 <?php
			...
			{
			?>
			{ y: '<?=fechahumana($dia["fecha"]);?>', a: <?=$dia["NUM_CANTIDAD"]?>, b: 90 },
		<?php 
			}?>
		  ],
		  xkey: 'y',
