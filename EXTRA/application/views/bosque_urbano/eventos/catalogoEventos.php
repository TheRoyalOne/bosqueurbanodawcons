...
<div class="row form-horizontal" style="padding-left:20px">
  <div class="col-lg-8 col-lg-offset-1 col-md-8 col-md-offset-1 col-sm-8 col-sm-offset-1">
	...
	<?php
	if(in_array(17,$this->session->userdata('PERMISOS')))
	{?>
	...
	?>
	<?php
	if(in_array(18,$this->session->userdata('PERMISOS')))
	{?>
	...
	?>
	<?php
	if(in_array(19,$this->session->userdata('PERMISOS')))
	{?>
      <button type="button" id="btnAdopcion" class="btn btn-primary" ><i class="fa fa-arrow-right" ></i> Adopcion sobre evento</button>            
    <?php
	}
    ?>
    </div>
    ...
    <div class="form-group" style="display:none">
      ...
      <div class="col-lg-8">
        <select class="form-control" id="VCH_TIPO" name="VCH_TIPO">
			<option value="1" <?php if($VCH_TIPO==1){?>selected="selected"<?php }?>>Adopcion</option>
        </select>
      </div>
    </div>
    <div class="form-group">
			 ...
			 <tr id="<?=$evento["ID__EVENTO"]?>"  >
				...
				<!-- <td>< ?=$evento["FEC_FECHAFIN"]?></td>			-->
				 ...
				 <td><?php 
					...
					<td>
							<?php
							if(($evento["VCH_ESTADOASIGNACION"]==2)&&($evento["VCH_ESTATUS"]==1))
							{?>
