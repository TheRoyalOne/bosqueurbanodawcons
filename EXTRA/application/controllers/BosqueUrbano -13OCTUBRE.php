...
class BosqueUrbano extends CI_Controller 
{
	...
	public function EmbajadoresInstitucion()
	{
		...
		$datos=array('titulo'=>'Instituciones/Embajadores','instituciones'=>$instituciones,'responsables'=>$responsables,'menu'=>'Configuración');
...
//		die("??");
	...
	{						
		$especies= $this->bosqueurbano_model->get_selectEspeciesDisponibles();				
	...
	public function AsignaArboladoEvento()
	{
		$ID__EVENTO=$this->input->post("ID__EVENTO");		
		$ID__USUARIO=$this->input->post("ID__USUARIO");		
		$FFEC_FECHAFIN=$this->input->post("FFEC_FECHAFIN");		
		$ID__ESPECIE=$this->input->post("ID__ESPECIE");		
		$ID__UBICACION=$this->input->post("ID__UBICACION");		
		$edad=$this->input->post("edad");		
		$contenedor_id=$this->input->post("contenedor_id");		
		$cantidadAsignar=$this->input->post("cantidadAsignar");		
		...
		$faltantes=$cantidadAsignar;		
		$Array_ID__INVENTARIO=$this->bosqueurbano_model->busquedaInventarioParaDescontarDeAsignacion($ID__UBICACION,$ID__ESPECIE,$contenedor_id,$edad);
		foreach($Array_ID__INVENTARIO as $row_id_inventario)
		{
			if($row_id_inventario["NUM_CANTIDAD"]>=$faltantes)	//Si la cantidad del inventario es mayor a lo que falta... (el mejor o ultimo de los casos)
			{				
				$this->bosqueurbano_model->reduccion_inventarioGlobal($row_id_inventario["ID__INVENTARIO"],$faltantes);//Disminuye de ese inventario... la cantidad que solicitaron
				break;
			}
			else //Pide mas de lo que tenemos asi que mata el primero.
			{								
				$this->bosqueurbano_model->reduccion_inventarioGlobal($row_id_inventario["ID__INVENTARIO"],$row_id_inventario["NUM_CANTIDAD"]);//Disminuye de ese inventario... la cantidad que solicitaron
				$faltantes = $faltantes-$row_id_inventario["NUM_CANTIDAD"];
			}												
		}		
	}
...
//		die(print_r($eventos));
		...
		$relarbolasignados=$this->bosqueurbano_model->get_relarbolasignados($id);
		$datos=array('titulo'=>'Eventos por asignar','eventos'=>$eventos,'personal'=>$personal,'vehiculos'=>$vehiculos,'prestadores'=>$prestadores,'personalASIGNADO'=>$personalASIGNADO,'vehiculosASIGNADO'=>$vehiculosASIGNADO,'prestadoresASIGNADO'=>$prestadoresASIGNADO,'VCH_TIPO'=>$eventos[0]["VCH_TIPO"],'herramientas'=>$herramientas);
		$this->load->view('inicio_archivo',$datos)
	...
	{		
		$JSON=$this->input->post('json');
		$this->bosqueurbano_model->RealizarAdopcionCiudadana($JSON);		
		die($JSON);
	}
...
//		echo "<pre/>";die(print_r($JSONSALIDA));
		...
		$solicitante=$this->session->userdata["logged_in"]["VCH_NOMBRE"]." ".$this->session->userdata["logged_in"]["VCH_APELLIDOPATERNO"]." ".$this->session->userdata["logged_in"]["VCH_APELLIDOMATERNO"];
		$data=array('data'=>$data,'solicitante'=>$solicitante,/*'cargadores'=>$CARGADORES,*/'adopciones'=>$adopciones);	
